...
namespace Burcat.API.Media
{
    ...
    {
        public static BurcatList<IPost> GetPosts(string? search) => [.. InterfaceExchange.DoPostSearch(
            from p in InterfaceOptions.GetSingleUse<IPost>()
            join mp in InterfaceOptions.GetSingleUse<MessagePost>() on p.Identifier equals mp.Identifier into mps from mp in mps.DefaultIfEmpty()
            where mp == null || mp.ResponseTo == null
            select p
            , search)];
        public static BurcatList<IPost> GetPostsFor(Member member, string? search)
        {
            ...
            return [.. InterfaceExchange.DoPostSearch(
            from pt in InterfaceOptions.GetSingleUse<IPost>()
            join mp in InterfaceOptions.GetSingleUse<MessagePost>() on pt.Identifier equals mp.Identifier into mps from mp in mps.DefaultIfEmpty()
            join pl in InterfaceOptions.GetSingleUse<Politeness>() on (Guid)pt.Politeness equals pl.Identifier
            where (Guid)pt.Owner != member.Identifier && (mp == null || mp.ResponseTo == null)
            && pl.Language <= tolerance.Language && pl.Racism <= tolerance.Racism && pl.Sexuality <= tolerance.Sexuality && pl.Violence <= tolerance.Violence
            select pt
            , search)];
        }
    ...
    public class TitlePost : BurcatObject, IPost
    {
        ...
        public TitlePost(BurcatIdentifier<Member> owner, BurcatIdentifier<Politeness> politeness, string title, string message, string? tags = null, DateTime? postDate = null) { Owner = owner; Politeness = politeness; Title = title; Message = message; Tags = tags; PostDate = postDate ?? DateTime.Now; }
    ...
    public class ImagePost : TitlePost
    {
        ...
        public ImagePost(BurcatIdentifier<Member> owner, BurcatIdentifier<Image> image, BurcatIdentifier<Politeness> politeness, string title, string message, string? tags = null, DateTime? postDate = null) : base(owner, politeness, title, message, tags, postDate) { Image = image; }
    ...
    public class VideoPost : BurcatObject, IPost
    {
        ...
        BurcatIdentifier<Politeness> IPost.Politeness => (from v in InterfaceOptions.GetSingleUse<Video>() where v == Video select v.Politeness).First();
