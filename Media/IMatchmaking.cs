...
namespace Burcat.API.Media
{
    ...
    public class MemberMatchmaking : BurcatObject, IMatchmaking
    {
        ...
        bool IInterfaceObject.ShouldManage(BurcatIdentifier<Member>? member) => member is not null && (from a in InterfaceOptions.GetSingleUse<AdvertisementPost>() join p in InterfaceOptions.GetSingleUse<IPost>() on (Guid)a.Post equals p.Identifier where a == Advertisement && p.Owner == member select true).Any();
