...
namespace Burcat.API
{
    ...
    {
        public static T? TryFind<T>(BurcatIdentifier<T> identifier) where T : IInterfaceObject => UseProvider(provider => (from o in provider.Get<T>() where o.Identifier == (Guid)identifier select o).FirstOrDefault());
        public static T Find<T>(BurcatIdentifier<T> identifier) where T : IInterfaceObject => UseProvider(provider => (from o in provider.Get<T>() where o.Identifier == (Guid)identifier select o).First());
