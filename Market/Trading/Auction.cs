...
namespace Burcat.API.Market.Trading
{
    ...
    public class Auction : BurcatObject, IGood
    {
        ...
        public bool ShouldManage(BurcatIdentifier<Member>? member) => member is not null && InterfaceOptions.UseProvider(provider => (from o in provider.Get<IOwnership>() where o.Identifier == (Guid)MadeOver && o.Owner == member select true).Any());
    ...
    public class CurrencyParticipationPrice : BurcatObject, ICurrencyPrice
    {
        ...
        public bool ShouldManage(BurcatIdentifier<Member>? member) => member is not null && InterfaceOptions.UseProvider(provider => (from a in provider.Get<Auction>() join o in provider.Get<IOwnership>() on (Guid)a.MadeOver equals o.Identifier where a == Auction && o.Owner == member select true).Any());
