...
namespace Burcat.API
{
    public static partial class DataValidator
    {
        ...
        {
            decimal? auctionParticipationPrice = (from a in InterfaceOptions.GetSingleUse<Auction>() where a == bid.MadeTo select a.ParticipationPrice).First();
            decimal memberLety = (from a in InterfaceOptions.GetSingleUse<Member>() where a == bid.MadeBy select a.Lety).First();
        ...
        {
            int currencyOwned = (from o in InterfaceOptions.GetSingleUse<CurrencyOwnership>() where o.Owner == bid.MadeBy select o.Amount).FirstOrDefault();
            CurrencyParticipationPrice? participationPrice = (from pp in InterfaceOptions.GetSingleUse<CurrencyParticipationPrice>() where pp.Auction == bid.MadeTo select pp).FirstOrDefault();
        ...
        {
            int amountOwned = (from o in InterfaceOptions.GetSingleUse<IOwnership>() where o.Identifier == auction.MadeOver.Value select o.Amount).FirstOrDefault();
